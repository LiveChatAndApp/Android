...
public class SettingActivity extends WfcBaseActivity {
    ...
    @BindView(R.id.versionOptionItemView)
    OptionItemView versionOptionItemView;
    ...
    protected void afterViews() {
        super.afterViews();
        versionOptionItemView.setDesc(BuildConfig.VERSION_NAME);
    }
        ...
        switch (requestCode) {
            case REQUEST_IGNORE_BATTERY_CODE:
                if (resultCode == RESULT_CANCELED) {
                    Toast.makeText(this, "允许" + getString(R.string.app_name) + "后台运行，更能保证消息的实时性", Toast.LENGTH_SHORT).show();
                }
                break;
            default:
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            try {
                ...
                if (!pm.isIgnoringBatteryOptimizations(packageName)) {
                    ...
                    Toast.makeText(this, "已忽略电池优化，允许" + getString(R.string.app_name) + "后台运行，更能保证消息的实时性", Toast.LENGTH_SHORT).show();
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
