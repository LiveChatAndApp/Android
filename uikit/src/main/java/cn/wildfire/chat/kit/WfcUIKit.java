...
public class WfcUIKit implements AVEngineKit.AVEngineCallback, OnReceiveMessageListener, OnRecallMessageListener, OnDeleteMessageListener, OnFriendUpdateListener {
    ...
    public void init(Application application) {
        ...
        LQREmotionKit.init(application, (context, path, imageView) ->
                GlideUtil.load(context, path).
                        apply(new RequestOptions().centerCrop().diskCacheStrategy(DiskCacheStrategy.RESOURCE).dontAnimate()).
                        into(imageView));
    ...
    private void initWFClient(Application application) {
        ChatManager.init(application, Security.decrypt(BuildConfig.IM_SERVER_HOST));
        ChatManager.Instance().setUserSource(ImplementUserSource.Instance()); // 不透过 IM server , 实作 我方用户资料server api
