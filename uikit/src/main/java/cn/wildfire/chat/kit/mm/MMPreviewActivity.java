...
public class MMPreviewActivity extends Activity {
    ...
    private void previewVideo(View view, MediaEntry entry) {
        ...
        if (entry.getThumbnail() != null) {
            GlideUtil.load(photoView, entry.getThumbnail())
                    .diskCacheStrategy(diskCacheStrategy)
                    .into(photoView);
        } else {
            GlideUtil.load(photoView, entry.getThumbnailUrl())
                    .diskCacheStrategy(diskCacheStrategy)
                    .into(photoView);
        }
            ...
            public void onClick(View v) {
                ...
                if (TextUtils.isEmpty(entry.getMediaLocalPath())) {
                    ...
                    if (!videoFile.exists() || secret) {
                        ...
                        String url = GlideUtil.checkHost(entry.getMediaUrl());
                        LogHelper.e(TAG, "视频网址 = " + url);
                        try {
                            DownloadManager.download(url, videoFile.getParent(), videoFile.getName(), new DownloadManager.OnDownloadListener() {
                                @Override
                            ...
                            Toast.makeText(MMPreviewActivity.this, "视频网址错误", Toast.LENGTH_SHORT).show();
                        } catch (Exception e) {
            ...
            if (file.exists() || secret) {
                ...
                saveImageView.setOnClickListener(v -> {
                    if (!file.getParentFile().exists()) {
                        ...
                        LogHelper.e("download", "file.getParentFile() path = " + file.getParentFile().getPath() + " , mkdirs = " + isSuccess);
                    }
                    saveImageView.setVisibility(View.GONE);
                    DownloadManager.download(mediaUrl, file.getParent(), file.getName(), new DownloadManager.SimpleOnDownloadListener() {
                        @Override
                        public void onUiSuccess(File file1) {
                            ...
                            // 更新系統相簿
                            ImageUtils.saveMedia2Album(MMPreviewActivity.this, file1);
