...
public class ConversationFragment extends Fragment implements
    ...
    private Observer<UiMessage> messageLiveDataObserver = new Observer<UiMessage>() {
        ...
        public void onChanged(@Nullable UiMessage uiMessage) {
            ...
            // video url "{{domain}}/static/image/upload/media/fs/3/2022/11/30/12/ZDFncW13czJr-3-1669782877-fai3scQuFCjN.mp4"
            // 发送讯息时，可以确认 ,messageId 来辨认是否是以送出的讯息
...
//            adapter.removeMessage();
    ...
    public void setupConversation(Conversation conversation, String title, long focusMessageId, String target) {
        ...
        if (checkIsAdmin(conversation.target) || checkIsAdmin(target)) {
    ...
    private boolean checkIsAdmin(String id) {
        if (!TextUtils.isEmpty(id) && "admin".equals(id)) {
            return true;
        } else {
            return false;
        }
    ...
    private void setupConversation(Conversation conversation) {
        ...
        userViewModel.getUserInfo(conversation.target, true);
        ...
        conversationViewModel.clearUnreadStatus(conversation); // 清除未读讯息记录
        ongoingCalls = null;
        setTitle();
    }
    ...
    private void quitChatRoom() {
        LogHelper.e(TAG, "quitChatRoom sendMessageEx");
        ...
        messageViewModel.sendMessageEx(message);
        ...
        if (conversation.type == Conversation.ConversationType.Single) {
            ...
            if (groupInfo != null) {
                if (groupInfo.isBroadCastGroup()) {
                    // 广播群
                    conversationTitle = (!TextUtils.isEmpty(groupInfo.remark) ? groupInfo.remark : groupInfo.name);
                } else {
                    // 普通群组
                    cutGroupTitle();
                }
    ...
    private void cutGroupTitle() {
        ...
        try {
            ...
            conversationTitle = text + "(" + groupInfo.memberCount + "人)";
        } catch (Exception e) {
            if (title.length() > 7) {
                title = title.substring(0, 6);
                title += "...";
            }
            conversationTitle = title + "(" + groupInfo.memberCount + "人)";
        }
    }
