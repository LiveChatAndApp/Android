...
public class ConversationViewHolder extends RecyclerView.ViewHolder {
    ...
    public void onBind(ConversationInfo conversationInfo) {
        ...
        if (conversation.type == Conversation.ConversationType.Single) {
            ...
            if (userInfo != null) {
                GlideUtil.load(fragment, userInfo.portrait).apply(new RequestOptions().centerCrop().placeholder(R.mipmap.avatar_def)).into(portraitImageView);
                nameTextView.setText(userViewModel.getUserDisplayName(userInfo));
            }
        } else {
            GroupInfo groupInfo = groupViewModel.getGroupInfo(conversation.target, false);
            if (groupInfo != null) {
                GlideUtil.load(fragment, groupInfo.portrait).apply(new RequestOptions().placeholder(R.mipmap.ic_group_default_portrait).centerCrop()).into(portraitImageView);
