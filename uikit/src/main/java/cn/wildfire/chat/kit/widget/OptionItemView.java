...
public class OptionItemView extends LinearLayout {
    ...
    private ConstraintLayout contactLinearLayout;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < typedArray.getIndexCount(); i++) {
            ...
            } else if (attr == R.styleable.OptionItemView_background_src) {
                int resId;
                resId = typedArray.getResourceId(attr, 0);
                if (resId != 0) {
                    contactLinearLayout.setBackgroundResource(resId);
                }
            ...
            } else if (attr == R.styleable.OptionItemView_desc_color) {
                int color = typedArray.getColor(attr, 0);
                descTextView.setTextColor(color);
            } else if (attr == R.styleable.OptionItemView_desc_size) {
            ...
            } else if (attr == R.styleable.OptionItemView_divider_visible) {
                boolean isShow = typedArray.getBoolean(attr, true);
                dividerView.setVisibility(isShow ? VISIBLE : GONE);
            } else if (attr == R.styleable.OptionItemView_contact_height) {
                int size = typedArray.getDimensionPixelSize(attr, 0);
                if (size != 0) {
                    contactLinearLayout.getLayoutParams().height = size;
                }
            }
        }
