...
public class ContactViewModel extends ViewModel implements OnFriendUpdateListener {
     ...
     * 查询好友邀请列表
     */
    public LiveData<List<AppServerFriendRequest>> getFriendRequestAppService() {
        MutableLiveData<List<AppServerFriendRequest>> liveData = new MutableLiveData<>();
        ImplementUserSource.Instance().getFriendRequest(new SimpleCallback<List<AppServerFriendRequest>>() {
            @Override
            public void onUiSuccess(List<AppServerFriendRequest> list) {
     ...
     */
    public LiveData<WebResponse<String>> sendFriendRequest(String uid, boolean verify, String verifyText, String message) {
        MutableLiveData<WebResponse<String>> liveData = new MutableLiveData<>();
        ImplementUserSource.Instance().sendFriendRequest(uid, verify, verifyText, message, new SimpleCallback<WebResponse<String>>() {
            @Override
     ...
     * @param verifyText 验证讯息
     */
    public LiveData<WebResponse<String>> responseFriendRequest(String uid, int reply, String verifyText) {
        MutableLiveData<WebResponse<String>> liveData = new MutableLiveData<>();
        ImplementUserSource.Instance().responseFriendRequest(uid, reply, verifyText, new SimpleCallback<WebResponse<String>>() {
            @Override
            public void onUiSuccess(WebResponse<String> response) {
                liveData.postValue(response);
            }
            ...
            @Override
            public void onUiFailure(int code, String msg) {
                liveData.postValue(new WebResponse(code, msg));
            }
        });
        return liveData;
    }
