...
public class ClientService extends Service implements SdtLogic.ICallBack,
    ...
    private static final String TAG = "ClientServiceService";
    ...
    private class ClientServiceStub extends IRemoteClient.Stub {
        ...
        public boolean connect(String userName, String userPwd) throws RemoteException {
            Log.e(TAG, "client connect:" + userName);
            if (logined) {
            ...
            Log.e("ClientService", " mHost = " + mHost);
            if (useSM4) {
...
//            }
        ...
        public void send(cn.wildfirechat.message.Message msg, final ISendMessageCallback callback, int expireDuration) throws RemoteException {
            ...
            if (msg.content instanceof MediaMessageContent) {
                android.util.Log.e(TAG, "msg localPath = "+((MediaMessageContent) msg.content).localPath);
                android.util.Log.e(TAG, "msg remoteUrl = "+((MediaMessageContent) msg.content).remoteUrl);
            ...
            if (uploadThenSend) {
                ...
                ProtoLogic.getUploadMediaUrl(file.getName(), MessageContentMediaType.FILE.getValue(), null, new ProtoLogic.IGetUploadMediaUrlCallback() {
                    ...
                    public void onSuccess(String uploadUrl, String remoteUrl, String backUploadupUrl, int serverType) {
                        android.util.Log.e(TAG,"uploadUrl = "+uploadUrl);
                        android.util.Log.e(TAG,"remoteUrl = "+remoteUrl);
                        android.util.Log.e(TAG,"backUploadupUrl = "+backUploadupUrl);
                        android.util.Log.e(TAG,"serverType = "+serverType);
                ...
                android.util.Log.e(TAG, "沒上傳檔案，直接發送");
                // 傳送訊息
        ...
        @Override
        public void getRemoteMessagesAndSave(Conversation conversation, int[] contentTypes, long beforeMessageUid, int count, IGetRemoteMessagesCallback callback) throws RemoteException {
            if (contentTypes == null) {
                contentTypes = new int[0];
            }
                ...
                public void onSuccess(ProtoMessage[] list) {
                    ...
                    try {
                        ...
                        for (Message message : messages) {
                            ProtoLogic.insertMessage(convertMessage(message));
                        }
                        int startIndex = 0;
                        do {
                            entry = buildSafeIPCEntry(messages, startIndex);
        ...
        public void createGroup(String groupId, String groupName, String groupPortrait, int groupType, String groupExtra, List<String> memberIds, String memberExtra, int[] notifyLines, MessagePayload notifyMsg, final IGeneralCallback2 callback) throws RemoteException {
            ...
            ProtoLogic.createGroup(groupId, groupName, groupPortrait, groupType, groupExtra, memberArray, memberExtra, notifyLines, notifyMsg == null ? null : notifyMsg.toProtoContent(), new ProtoLogic.IGeneralCallback2() {
                ...
                public void onSuccess(String s) {
                    try {
                        android.util.Log.e(TAG, "createGroup onSuccess = " + s);
                        callback.onSuccess(s);
                    } catch (RemoteException e) {
                        e.printStackTrace();
                    }
                ...
                public void onFailure(int i) {
                    try {
                        android.util.Log.e(TAG, "onFailure onSuccess = " + i);
                        callback.onFailure(i);
                    } catch (RemoteException e) {
                        e.printStackTrace();
                    }
        ...
        public String getHost() throws RemoteException {
            String url = StnLogic.getHost();
            return url;
        }

        @Override
        public int getPort() throws RemoteException {
            int port = StnLogic.getPort();
            return port;
        }

        @Override
        public String getHostEx() throws RemoteException {
            String host = StnLogic.getHostEx();
            return host;
        }
    ...
    private boolean initProto(String userName, String userPwd) {
        ...
        Log.e(TAG, "Proto connect:" + userName);
...
//        }
    ...
    public void onUserInfoUpdated(List<ProtoUserInfo> list) {
        android.util.Log.e(TAG, "onUserInfoUpdated");
        handler.post(() -> {
