...
public class GroupInfo implements Parcelable {
    ...
    public boolean isBroadCastGroup() {
        if (extra == null) {
            ...
            if (getExtraInfo() == null) {
                ...
                return getExtraInfo().groupType == 2;
            }
        }
    }

    private ExtraInfo parseExtra() {
        if (TextUtils.isEmpty(extra)) {
            return null;
        }
        Gson gson = new Gson();
        JsonObject jsonExtra = gson.fromJson(extra, JsonObject.class);
        Type extraInfoType = new TypeToken<ExtraInfo>() {
        }.getType();
        return gson.fromJson(jsonExtra, extraInfoType);
    }

    public ExtraInfo getExtraInfo() {
        if (extraInfo == null) {
            extraInfo = parseExtra();
        }
        return extraInfo;
    }
    ...
    /**
     * extra:{"creator":"lance","creatorRole":1,"groupType":1}
     */
